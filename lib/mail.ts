...
export async function sendOrderEmail(orderNo: string) {
  try {
    ...
    // 2. Fetch Email configuration
    const settings = await prisma.systemSetting.findMany({
      where: {
        key: {
          in: [
            'resend_api_key', 'resend_from_email', 'resend_enabled',
            'aliyun_smtp_host', 'aliyun_smtp_port', 'aliyun_smtp_user', 'aliyun_smtp_pass', 'aliyun_from_email', 'aliyun_enabled',
            'site_title'
          ]
        }
      }
    ...
    // Explicitly type parameters to fix lint error
    const licenseList = order.licenses
      .map((l: any, i: number) => formatLicense(l.code, i, deliveryFormat))
      .join('\n');
